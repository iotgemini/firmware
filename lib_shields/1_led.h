...
#ifdef UART_DEBUG
	//#define UART_DEBUG_LED_1 //activate the debug on the UART for this library
#endif
...
#define NUM_IO_LED_1			1		//it is the less significant byte of the univocal identifier
...
//---------------------------------------------------------------------------------
	...
	void 			init_led_1(void);					//Function that allocate the variables and init the shield

	#ifdef UART_DEBUG_LED_1
	void 	UART_DEBUG_LED_1_send_STR(unsigned char num_data, unsigned char *data, unsigned char line_feed);//it send a string through the UART only if is in debug mode
	#endif
//---------------------------------------------------------------------------------
//	END LIST FUNCTIONS
//---------------------------------------------------------------------------------
